...
function getMenuHTML(rest){
	...
	Object.keys(menu).forEach(key =>{
		...
		Object.keys(menu[key]).forEach(id => {
			item = menu[key][id];
			result += `${item.name} (\$${item.price}) <button onclick='addItem(${id})'>+</button><br>`;
...
function updateOrder(){
	...
	Object.keys(order).forEach(id =>{
		...
		result += `${item.name} x ${order[id].quantity} (${(item.price * order[id].quantity).toFixed(2)}) <button onclick='removeItem(${id})'>-</button><br>`;
	});
...
//Function that will submit a order and update the logged in user's order history.
function submitOrder(){ 

	let x = new XMLHttpRequest();
	x.open("PUT", "http://localhost:3000/orderupdate");
	...
	x.send(JSON.stringify(orderInfoToSend));
	
	alert("Order has been placed.");
	//Reset's order info locally.
	order = {};
	updateOrder();
}